    ...
    if (sectionId === 'products') {
        ...
        if (user) {
            if (typeof loadAddresses === 'function') {
                ...
                console.error('loadAddresses function not found');
            }
        } else {
            showToast('Please login to view your addresses', 'error');
            navigateTo('home');
        }
    } else if (sectionId === 'checkout') {
        if (user) {
            if (typeof renderCheckoutOrderSummary === 'function') {
                setTimeout(renderCheckoutOrderSummary, 50);
            }
            if (typeof renderCheckoutAddressSelection === 'function') {
                setTimeout(renderCheckoutAddressSelection, 50);
            }
        } else {
